...
FastAPI remains excellent for one specific use case: ML inference pipelines where the Python ecosystem is non-negotiable. NumPy[N AH1 M P AY0], PyTorch, Hugging Face integrations — if your API is fundamentally calling Python ML code, FastAPI is the right call. For everything else, Bun covers the "fast to ship" lane with competitive throughput and a much smaller footprint than a Python process.

## The Takeaway

There's no wrong answer inside the triangle. Rust, Bun, and ASP.NET[AE1 S P IH0 DAA1 T N EH1 T] each have a clear job and a defensible case. What's harder to defend is defaulting to Go or FastAPI out of habit — choosing familiarity over fit.
