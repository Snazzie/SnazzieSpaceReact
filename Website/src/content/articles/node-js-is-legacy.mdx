---
title: "Node.js Is Legacy"
date: 2026-06-26
excerpt: "Node isn't bad. It's just no longer the default that wins on merit. Bun took that title and isn't giving it back."
tags: ["bun", "node", "javascript", "performance", "tooling"]
draft: false
---
...
Because look at *where* Bun won. Server throughput. Large JSON parsing. The HTTP[EY1 CH, T IY1, T IY1, P IY1] path. The exact operations a real backend does ten thousand times a second while a request waits. Bun didn't edge those out on a technicality, it stood out most on precisely the work you deploy a runtime to do.
...
[snazzie.space](https://snazzie.space) runs on Bun. The build, the test suite, the package install, the dev server, all of it, one tool. The backend is [Hono](https://hono.dev) on a Cloudflare Worker, the frontend is [Astro](https://astro.build), and the runtime underneath never once made me open a config file to apologize for it.

I didn't switch for the benchmark. I switched because bun install finishes before I've alt-tabbed away, and because I stopped maintaining the pile of dev dependencies that existed only to do what one binary now does for free. The speed was a bonus I noticed later.
